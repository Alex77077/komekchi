...
function Dash({ workers, tasks, attend, depts, C, mob, cu, settings, tl }) {
  ...
  const act = [
    ...todA.filter((a) => a.check_out).map((a) => {
      const w = workers.find((x) => x.id === a.wid);
      return { t: `${w ? w.name : "?"} ${tl.leftWork}`, time: a.check_out, ic: "🚪" };
    }),
    ...todA.filter((a) => a.check_in).map((a) => {
      const w = workers.find((x) => x.id === a.wid);
      const isLate = tMin(a.check_in) > tMin(settings.workStart) + settings.lateLimit;
      return { t: `${w ? w.name : "?"} ${isLate ? tl.lateArrival : tl.cameIn}`, time: a.check_in, ic: I.check(C.pu,22) };
    }),
...
function Attend({ workers, attend, setAttend, setWorkers, C, mob, cu, settings, toast, tl }) {
  ...
  const doIn = async (wid) => {
    ...
    // "Irden" diýmek: iş başlanmanka 30 min-dan köp gelen
    const earlyMin = 30;
...
function KanbanCard({ task, workers, onEdit, onDelete, onMove, onDetail, C, cu, tl }) {
  ...
  return (
    <div className="kc" style={{ background: C.sf, border: `1px solid ${isOv ? C.rd + "66" : C.bd}`, borderRadius: 14, padding: 13, borderTop: `3px solid ${isOv ? C.rd : isDu ? C.yw : task.clr}`, transition: "all .2s" }}>
      ...
      {task.dl && task.col !== "Tamamlandy" && (
        <div style={{ fontSize: 11, color: isOv ? C.rd : isDu ? C.yw : C.txS, marginBottom: 5, fontWeight: isOv || isDu ? 700 : 400 }}>
...
function Kanban({ tasks, setTasks, workers, C, mob, cu, toast, tl }) {
  ...
  const moveTask = async (id, col) => {
    try {
      const patch = { col };
      if (col === "Tamamlandy") patch.completed_at = gToday();
      await sbFetch(`tasks?id=eq.${id}`, "PATCH", patch);
